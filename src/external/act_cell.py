...
class ACTCell(RNNCell):
    ...
    def __call__(self, inputs, state, timestep=0, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__):
            ...
            prob = tf.zeros(self.batch_size, name="prob")
            prob_compare = tf.zeros(self.batch_size, name="prob_compare")
            counter = tf.zeros(self.batch_size, name="counter")
            acc_outputs = tf.zeros_like(state, tf.float32, name="output_accumulator")
            acc_states = tf.zeros_like(state, tf.float32, name="state_accumulator")
            batch_mask = tf.cast(tf.fill(self.batch_size, True), tf.bool)
    ...
    def act_step(self,batch_mask,prob_compare,prob,counter,state,input,acc_outputs,acc_states):
        ...
        with tf.variable_scope('sigmoid_activation_for_pondering'):
            input_shapes = [x.get_shape() for x in list(new_state)]
            print(input_shapes)
